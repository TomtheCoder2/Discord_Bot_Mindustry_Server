...
class SlashCreator extends eventemitter3_1.default {
    ...
    constructor(opts) {
         ...
         * @private
         */
        this._awaitingCommandCtxs = new Map();
    ...
    registerCommand(command) {
        ...
        if (command.guildIDs &&
            this.commands.some((cmd) => !!(cmd.commandName === command.commandName &&
                cmd.guildIDs &&
    ...
    async collectCommandIDs(skipGuildErrors = true) {
        ...
        for (const guildID of guildIDs) {
            try {
                ...
                if (skipGuildErrors) {
                    this.emit('warn', `An error occurred during guild command ID collection (${guildID}), you may no longer have access to that guild.`);
                }
                else {
                    throw e;
                }
    ...
    cleanRegisteredComponents() {
        if (this._awaitingCommandCtxs.size)
            Array.from(this._awaitingCommandCtxs.keys()).forEach((messageID) => {
                ...
                if (ctx.expired)
                    this._awaitingCommandCtxs.delete(messageID);
            });
    }
        ...
        switch (interaction.type) {
            ...
            case constants_1.InteractionType.COMMAND: {
                ...
                if (!command) {
                    ...
                    if (this.unknownCommand) {
                        const ctx = new context_1.default(this, interaction, respond, webserverMode, this.unknownCommand.deferEphemeral);
                  ...
                  `,
                    ...
                    const ctx = new context_1.default(this, interaction, respond, webserverMode, command.deferEphemeral);
            ...
            case constants_1.InteractionType.MESSAGE_COMPONENT: {
                ...
                if (this._awaitingCommandCtxs.size || this.listenerCount('componentInteraction') > 0) {
                    const ctx = new componentContext_1.default(this, interaction, respond);
